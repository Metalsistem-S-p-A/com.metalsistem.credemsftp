...
public class FromCredemProcess extends SvrProcess {
	...
	private Integer fattureImportate = 0;
	private Integer fatturePresenti = 0;
		...
		try (final SSHClient ssh = new SSHClient()) {
			...
			for (RemoteResourceInfo entry : filelist) {
				...
				if (credemId.equals(parts[1])) {
					...
					if (inv != null) {
						salvaInvoice(inv);
						archiviaEInvoice(xml, inv);
					}
				}
			}
			sftp.close();
			ssh.close();
			return "Importate " + fattureImportate + " fatture, Scartate: " + (fatturePresenti - fattureImportate);
		}
	}

	private void archiviaEInvoice(byte[] xml, MInvoice inv) {
		ME_Invoice einv = new Query(getCtx(), ME_Invoice.Table_Name, "Name = ?", null).setClient_ID()
				.setParameters("FE: " + inv.getDocumentNo()).first();
		if (einv == null) {
	...
	public void salvaInvoice(InvoiceReceived inv) throws Exception {

		MInvoice res = new Query(getCtx(), MInvoice.Table_Name, "DocumentNo = ?", null).setClient_ID()
				.setParameters(inv.getDocumentNo()).first();
		if (res == null) {
			inv.saveEx();
			fattureImportate++;
	...
	public InvoiceReceived getInvoice(FatturaElettronicaType fattura, boolean fornitore) throws Exception {
		...
		DatiGeneraliDocumentoType datiGeneraliDocumento = firstBody.getDatiGenerali().getDatiGeneraliDocumento();
		...
		if (!List.of(TipoDocumentoType.TD_04, TipoDocumentoType.TD_16, TipoDocumentoType.TD_17, TipoDocumentoType.TD_18)
			...
			docType = new Query(getCtx(), MDocType.Table_Name, "lit_fepa_doctype = ? and issotrx='N' ", null)
					.setClient_ID().setParameters(datiGeneraliDocumento.getTipoDocumento().value()).first();
			if (docType != null) {
				invoice.setC_DocType_ID(docType.get_ID());
				invoice.setC_DocTypeTarget_ID(docType.get_ID());
			} else {
				docType = new Query(getCtx(), MDocType.Table_Name, "lit_fepa_doctype = 'TD01' and issotrx='N' ", null)
						.setClient_ID().first();
				invoice.setC_DocType_ID(docType.get_ID());
				invoice.setC_DocTypeTarget_ID(docType.get_ID());
			}
		...
		MBPartner mbp = new Query(getCtx(), MBPartner.Table_Name, "LIT_TaxId=?", null).setClient_ID()
				.setParameters(codice).first();
...
//		MLocation bpLocation = MLocation.getBPLocation(getCtx(), mbp.getPrimaryC_BPartner_Location_ID(), null);
		...
		if (letter != null && body.getDatiBeniServizi().getDatiRiepilogo().get(0).getNatura() != null
		...
		for (DettaglioLineeType linea : body.getDatiBeniServizi().getDettaglioLinee()) {
			...
			il.setLine(linea.getNumeroLinea() * 10); // Standard iDempiere
			...
			if (mbp.get_ValueAsInt("LIT_M_Product_XML_ID") > 0) {
				MProduct prod = new MProduct(getCtx(), mbp.get_ValueAsInt("LIT_M_Product_XML_ID"), null);
				il.setProduct(prod);
			}
		...
		if (datiPagamento.size() > 0) {
			...
			ModalitaPagamentoType dpt = datiPagamentoType.getDettaglioPagamento().get(0).getModalitaPagamento();
		...
		for (DatiPagamentoType pagamento : body.getDatiPagamento()) {
			for (DettaglioPagamentoType dettaglio : pagamento.getDettaglioPagamento()) {
				...
				if (dataScadenza != null) {
					ips.setDueDate(new Timestamp(dataScadenza.toGregorianCalendar().getTimeInMillis()));
					ips.setDiscountDate(new Timestamp(dataScadenza.toGregorianCalendar().getTimeInMillis()));
					scadenze.add(ips);
				}
				// In caso non ci siano date, vengono generate
				// inidempiere al momento del completamento della fattura
			}
		}
	...
	private MTax getTax(final MLITVATDocTypeSequence registroIva, DettaglioLineeType linea) {
		List<MTax> taxes = new Query(getCtx(), MTax.Table_Name, "to_country_id = 214 ", null).setClient_ID().list();
		MTax invTax = null;
		if (linea.getNatura() != null && "N3.5".equals(linea.getNatura().value())) {
			invTax = new Query(getCtx(), MTax.Table_Name, "value = 'F.02'", null).setClient_ID().first();
		} else {
	...
	private MBPartnerLocation getBPLocationFromEinvoice(FatturaElettronicaType fattura) {
		...
		MLocation location = new Query(getCtx(), MLocation.Table_Name,
				"c_location.address1 like ? AND c_location.postal = ? AND c_location.city = ? and co.countrycode = ? and reg.name = ?",
				...
				.setParameters(sede.getIndirizzo(), sede.getCAP(), sede.getComune(), sede.getNazione(),
						sede.getProvincia())
				.first();
		if (location == null) {
			...
			if (country != null) {
				...
				MRegion region = new Query(getCtx(), MRegion.Table_Name, "name = ? AND c_country_id = ?", null)
	...
	private MBPartner createAndSaveBusinessPartner(FatturaElettronicaType fattura, String codice) {
		...
		mbp.setName(fattura.getFatturaElettronicaHeader().getCedentePrestatore().getDatiAnagrafici().getAnagrafica()
				.getDenominazione() + " - TEST");
	...
	private void publishNewBpMessage(MBPartner mbp) {
		...
		MRole role = new Query(getCtx(), MRole.Table_Name, "name = 'Amministrazione (responsabili)'", null).setClient_ID().first();
		if (role == null) {
			role = new Query(getCtx(), MRole.Table_Name, "name like 'Amministratore%'", null).setClient_ID().first();
		}
		msg.setBroadcastMessage("Business Partner creato per " + mbp.getName());
	...
	public int parsePaymentTerm(String id) {
		...
		MPaymentTerm term = new Query(getCtx(), MPaymentTerm.Table_Name, "Name = ?", null)
				.setParameters(terminePagamento).setClient_ID().first();
