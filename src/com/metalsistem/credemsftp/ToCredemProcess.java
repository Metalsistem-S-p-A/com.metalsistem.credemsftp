 ...
 * @author jtomelleri
 * 
 * This class handles the process of exporting invoices to Credemtel via SFTP.
 * It generates CSV files for PO invoices and ZIP files containing XML for SO invoices.
 * The files are then uploaded to a specified SFTP server.
 *
 * <p>Key functionalities of this class include:
 * - Reading invoice data from the database.
 * - Generating CSV and ZIP files based on invoice data.
 * - Uploading generated files to a remote SFTP server.
 * - Marking invoices as synchronized once processed and uploaded.
 *
 ...
 * - SiaCode: Identifier used for naming the files.
 * - CsvIdentifier: Identifier for CSV file naming.
 * - ZipIdentifier: Identifier for ZIP file naming.
 * - Path: Path on the SFTP server where files will be uploaded.
 *
 * <p>Note: The process ensures data consistency by marking synchronized invoices
 * in the database to avoid duplicate processing.
 */
@Process
    ...
    protected String doIt() throws Exception {
        try (final SSHClient ssh = new SSHClient()) {
            ...
            List<MInvoice> poInvoices = new Query(getCtx(), MInvoice.Table_Name,
                    "isSoTrx='N' AND DocStatus = 'CO' AND isActive='Y'", null).setClient_ID()
                    .list();
            ...
            for (MInvoice inv : poInvoices) {
                ...
                ME_Invoice einv = new Query(getCtx(), ME_Invoice.Table_Name, "C_Invoice_ID=?", null)
                        .setClient_ID()
                        .setParameters(inv.getC_Invoice_ID())
                        .first();
                if (isValidRecord(inv, bp, doctype, einv)) {
                    csvPrinter.printRecord("IT" + bp.getTaxID(),
                            bp.get_Value("LIT_NationalIdNumber"), inv.getDocumentNo(),
                            inv.getDateInvoiced()
                                    .toLocalDateTime()
